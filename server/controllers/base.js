...
class baseController {
  ...
  async getLoginStatus(ctx) {
    let body;
    // 先检查之前是否有链接，有的话先处理链接(用回旧的链接)，没的话就鉴权登录
    ...
    if (!name) {
        name = ctx.cookies.get("km_uid");
    }
    let uidResult = await userInst.findByName(name);

    if (uidResult) {
      this.setLoginCookie(uidResult._id, uidResult.passsalt);
      return (ctx.body = yapi.commons.resReturn(
        { // 必须要将id绑定回去，要不会解析不了
          _id: uidResult._id,
          username: uidResult.username,
          role: uidResult.role,
          uid: uidResult._id,
          email: uidResult.email,
          add_time: uidResult.add_time,
          up_time: uidResult.up_time,
          type: 'site',
          study: uidResult.study
        },
        0,
        'logout success...'
      ));
