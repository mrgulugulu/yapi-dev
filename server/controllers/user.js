...
class userController extends baseController {
  ...
  async login(ctx) {
    ...
    //   ));
    // }
    // 它的逻辑应该如下，如果是自己组内的同事，就进入；如果不是，则需要它输入账号密码
    if (result) {
      this.setLoginCookie(result._id, result.passsalt);
      return (ctx.body = yapi.commons.resReturn(
        {
    ...
    let emailResult = await userInst.findByEmail(email);
    ...
    if (!emailResult) {
      return (ctx.body = yapi.commons.resReturn(null, 404, '该用户不存在'));
    } else if (yapi.commons.generatePassword(password, emailResult.passsalt) === emailResult.password) {
      this.setLoginCookie(emailResult._id, emailResult.passsalt);
      ...
      return (ctx.body = yapi.commons.resReturn(
        {
          ...
          add_time: emailResult.add_time,
          up_time: emailResult.up_time,
          type: 'site',
          study: emailResult.study
        },
        0,
        'logout success...'
      ));
